...
import {generateOutreachScript, type GenerateOutreachScriptInput} from '@/ai/flows/generate-outreach-script';
...
export default function CampaignPage() {
  ...
  const handleScheduleGoogleCalendarEvent = async () => {
    ...
    const eventDetails: GoogleCalendarEvent = {
      ...
      attendees: calendarEventAttendees
        .split(',')
      ...
      if (createdEvent) {
        toast({
          ...
          description: `Event "${createdEvent.summary}" created successfully. ${
            createdEvent.hangoutLink ? `Meet Link: ${createdEvent.hangoutLink}` : ''
          }`,
        });
  ...
  useEffect(() => {
    const fetchLinkedInProfile = async () => {
      if (platform === 'linkedin' && linkedinUsername) {
        // Added check for linkedinUsername
        try {
        ...
        setLinkedInProfile(null); // Clear the profile when platform changes or username is empty
      }
    };

    const fetchTwitterProfile = async () => {
      if (platform === 'twitter' && twitterUsername) {
        // Added check for twitterUsername
        try {
        ...
        setTwitterProfile(null); // Clear the profile when platform changes or username is empty
      }
    };

    const fetchEmailProfile = async () => {
      if (platform === 'email' && emailAddress) {
        // Added check for emailAddress
        try {
        ...
        setEmailProfile(null); // Clear the profile when platform changes or emailAddress is empty
      }
    };
  ...
  const handleGenerateTemplate = async () => {
    ...
    if (platform === 'linkedin' && linkedinUsername && (!linkedinProfile?.headline || !linkedinProfile?.profileUrl)) {
      // Enrich LinkedIn profile if data is insufficient and username is provided
      try {
        const enrichedProfileResult: EnrichLinkedInProfileOutput = await enrichLinkedInProfile({
          ...
          company: companyName,
          linkedinProfile: linkedinProfile || undefined, // Pass current profile or undefined
          additionalContext: additionalContext,
        });
        scriptToSet = enrichedProfileResult.enrichedProfile;
        // Optionally update linkedinProfile state if the enriched data is in the correct format
        // setLinkedInProfile({ id: linkedinProfile?.id || '', headline: 'Enriched Headline', profileUrl: 'Enriched URL' });
        toast({
          title: 'Success',
        ...
        toast({
          title: 'Error',
          description: 'Failed to enrich LinkedIn profile. Using existing data if available.',
          variant: 'destructive',
        });
        // Fallback to using potentially incomplete profile data if enrichment fails
        if (linkedinProfile) {
    ...
    const input: GenerateOutreachScriptInput = {
      platform: platform,
      linkedinProfile: platform === 'linkedin' && profileData ? { // Ensure profileData is for LinkedIn
        id: profileData.id,
    ...
    try {
      if (!scriptToSet) { // If script wasn't set by enrichment
        const result = await generateOutreachScript(input);
        scriptToSet = result.script;
      }
      setMessage(scriptToSet);
      setTemplates(prevTemplates => [...prevTemplates, {platform: platform, template: scriptToSet}]);
      toast({
        title: 'Success',
        description: 'Successfully generated outreach script.',
      });
       if (sentimentScore && sentimentScore > 0.5 && includeCallToAction) {
        toast({
          title: 'High Sentiment Detected!',
          description: 'Consider scheduling a virtual call with this prospect.',
        });
      }
    } catch (error) {
  ...
  // Placeholder for sentiment score state
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          ...
          {platform === 'whatsapp' && (
            <div className="grid gap-2">
              ...
              <Textarea
                ...
                onChange={e => setEmailAddress(e.target.value)}
                placeholder="Enter WhatsApp Number with country code (e.g., +1XXXXXXXXXX)"
              />
            </div>
          )}
          ...
          <div className="grid gap-2">
            ...
            <Textarea id="message" value={message} onChange={e => setMessage(e.target.value)} placeholder="Your message will appear here" />
          </div>
          <div className="grid gap-2">
                ...
                {!isGoogleCalendarAuthorized ? (
                  ...
                  <>
                    <div className="grid gap-2">
                      ...
                      <Input
                        id="eventSummary"
                        value={calendarEventSummary}
                        onChange={e => setCalendarEventSummary(e.target.value)}
                        placeholder="Meeting with Prospect"
                      />
                    </div>
                    <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        id="eventAttendees"
                        value={calendarEventAttendees}
                        onChange={e => setCalendarEventAttendees(e.target.value)}
                        placeholder="prospect@example.com, colleague@example.com"
                      />
                    </div>
                     <div className="flex items-center space-x-2">
                      <Checkbox
